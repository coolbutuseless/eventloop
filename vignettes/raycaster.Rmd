...
angle_deg <- c(30, 130, 230, 330)
angle_deg <- seq(0, 359, 5)
...
dists <- sqrt( (x - x0)^2 + (y - y0)^2 )
dists <- aggregate(dists, by = list(ray = ray), min)
...
grid.polyline(c(rbind(x0, x1)), c(rbind(y0, y1)), default.units = dunit,
           gp = gpar(lty = 2), id = rep(seq(Nangs), each = 2))
