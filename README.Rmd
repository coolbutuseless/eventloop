...
Only the `x11` device on macOS and *nix platforms includes an `onIdle()`
event callback which is necessary for `eventloop` to work.
...
If your code can run fast enough in R, then you can do some more complex rendering.
